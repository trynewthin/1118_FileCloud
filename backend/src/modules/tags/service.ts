...
// 获取所有标签（树形结构）
export const listAllTags = (): FileTag[] => {
  const rows = db
    .prepare("SELECT * FROM file_tags ORDER BY level ASC, sort_order ASC, name ASC")
    .all() as any[];
...
// 获取所有标签（扁平列表）
export const listAllTagsFlat = (): FileTag[] => {
  const rows = db
    .prepare("SELECT * FROM file_tags ORDER BY level ASC, sort_order ASC, name ASC")
    .all() as any[];
...
// 获取指定父标签下的子标签
export const listChildTags = (parentTagId: number | null): FileTag[] => {
  const rows = parentTagId === null
    ? db.prepare("SELECT * FROM file_tags WHERE parent_tag_id IS NULL ORDER BY sort_order ASC, name ASC").all() as any[]
    : db.prepare("SELECT * FROM file_tags WHERE parent_tag_id = ? ORDER BY sort_order ASC, name ASC").all(parentTagId) as any[];
  return rows.map(mapRowToTag);
};
...
export const createTag = (input: CreateTagInput): FileTag => {
  const { name, parentTagId, color, allowMultiple, sortOrder } = input;
  
  // 计算层级
  let level = 1;
  if (parentTagId) {
    const parent = getTagById(parentTagId);
    if (!parent) {
  ...
  const result = db
    .prepare(
      "INSERT INTO file_tags (name, parent_tag_id, level, color, allow_multiple, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    )
    .run(
      name.trim(),
      parentTagId ?? null,
...
export const updateTag = (id: number, input: UpdateTagInput): FileTag => {
  ...
  if (name !== undefined && name.trim() !== tag.name) {
    const existing = tag.parent_tag_id === null
      ? db.prepare("SELECT id FROM file_tags WHERE parent_tag_id IS NULL AND name = ? AND id != ?").get(name.trim(), id) as any | undefined
      : db.prepare("SELECT id FROM file_tags WHERE parent_tag_id = ? AND name = ? AND id != ?").get(tag.parent_tag_id, name.trim(), id) as any | undefined;
...
export const getTagsForEntry = (entryId: string): (FileTagEntry & { tag: FileTag })[] => {
  const rows = db
    .prepare(
      `SELECT fte.*, ft.name, ft.parent_tag_id, ft.level, ft.color, ft.allow_multiple, ft.sort_order, ft.created_at as tag_created_at, ft.updated_at as tag_updated_at
...
// 获取所有标签（带统计信息）
export const listAllTagsWithStats = (): FileTag[] => {
