...
import { abortStreamRequest, registerStreamAbort, unregisterStreamAbort } from "./stream/abortRegistry.ts";
  ...
  async (req, res) => {
    ...
    if (hasAttachments) {
      validAttachmentIds = attachmentIds.filter(
        (aid) => Number.isInteger(aid) && aid > 0
      );
    }

    // 生成请求 ID（如果前端未提供）
    const finalRequestId = requestId || `req_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
